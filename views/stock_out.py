            ...
            if not items_df.empty:
                with st.form("stock_out_form"):
                    selected_item_name = st.selectbox(
                        "Select Item to Issue*", items_df["item_name"].tolist()
                    )

                    # Fetch selected item's current stock details for initial UI rendering
                    item_row = items_df[
                    ...
                    col_info3.metric(
                        "Minimum Threshold Alert",
                        f"{min_thresh:,.2f} {unit}",
                    )

                    if current_available <= 0:
                        st.error(
                            f"⚠️ **{selected_item_name}** is currently OUT OF STOCK. Stock Out is disabled."
                        )
                    ...
                    submit_btn = st.form_submit_button(
                        "📤 Submit Stock Out", use_container_width=True
                    )

                    if submit_btn:
                        if not recipient.strip():
                            st.error(
                                "⚠️ Please specify the recipient, department, or project taking the materials."
                            )
                        elif quantity_out <= 0:
                            ...
                            try:
                                with get_db() as conn_trans:
                                    ...
                                    # 1. Atomic database deduction with stock sufficiency check
                                    cursor.execute(
                                        """
                                        UPDATE master_items 
                                    ...
                                    # If no rows were affected, inventory was insufficient at execution time
                                    if cursor.rowcount == 0:
                                        # Query fresh live stock to display accurate error context
                                        cursor.execute(
                                        ...
                                        # Fetch updated remaining balance
                                        cursor.execute(
                                            "SELECT current_stock FROM master_items WHERE item_name = ?",
                                            (selected_item_name,),
                                        )
                                        ...
                                        st.success(
                                            f"✅ Issued **{quantity_out:,.2f} {unit}** of **{selected_item_name}**. "
                                            f"Remaining balance: **{updated_stock:,.2f} {unit}**."
                                        )

                                        # Low Stock Threshold Warning Check
                                        if updated_stock <= min_thresh:
                                            st.warning(
                                                f"🔔 **Low Stock Alert**: {selected_item_name} has dropped to or below its threshold "
                                                f"({updated_stock:,.2f} / {min_thresh:,.2f} {unit})."
                                            )
