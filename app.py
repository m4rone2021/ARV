...
for col, col_type in [
  ...
  try:
    cursor.execute(
        f"ALTER TABLE physical_inventory_counts ADD COLUMN {col} {col_type}"
    )
  except sqlite3.OperationalError:
    pass
...
# Seed Expanded Inventory List Helper
...
def generate_excel_report(user_name, selected_date, df_daily_tx, df_current_stock):
  ...
  if not df_daily_tx.empty:
    for _, r in df_daily_tx.iterrows():
      ws_master.cell(
          row=row_idx, column=1, value=str(r.get("timestamp", ""))
      )
      ws_master.cell(
          row=row_idx, column=2, value=str(r.get("item_name", ""))
      )

      t_cell = ws_master.cell(
          row=row_idx, column=3, value=str(r.get("type", ""))
      )
      t_cell.alignment = Alignment(horizontal="center")
      t_cell.font = Font(
          bold=True,
          color="006100" if r.get("type") == "IN" else "9C0006",
      )

      q_cell = ws_master.cell(
          row=row_idx, column=4, value=float(r.get("quantity", 0))
      )
      q_cell.number_format = "#,##0.00"

      ws_master.cell(
          row=row_idx, column=5, value=str(r.get("issued_to", "-"))
      )
      ws_master.cell(
          row=row_idx, column=6, value=str(r.get("driver_details", "-"))
      )
      ws_master.cell(
          row=row_idx, column=7, value=str(r.get("project_name", "-"))
      )
      ws_master.cell(
          row=row_idx,
          column=8,
          value=str(r.get("purpose", r.get("remarks", "-"))),
      )
      ws_master.cell(
          row=row_idx, column=9, value=str(r.get("user_role", ""))
      )
  ...
  df_in = (
      df_daily_tx[df_daily_tx["type"] == "IN"]
      if not df_daily_tx.empty
      else pd.DataFrame()
  )
  row_idx = 5
  if not df_in.empty:
    for _, r in df_in.iterrows():
      ...
      q_cell = ws_in.cell(
          row=row_idx, column=3, value=float(r.get("quantity", 0))
      )
      q_cell.number_format = "#,##0.00"
  ...
  df_out = (
      df_daily_tx[df_daily_tx["type"] == "OUT"]
      if not df_daily_tx.empty
      else pd.DataFrame()
  )
  row_idx = 5
  if not df_out.empty:
    for _, r in df_out.iterrows():
      ...
      ws_out.cell(
          row=row_idx, column=5, value=str(r.get("driver_details", "-"))
      )
  ...
  if not df_current_stock.empty:
    for cat in sorted(df_current_stock["category"].unique()):
      ws_cat.merge_cells(
          start_row=c_row, start_column=1, end_row=c_row, end_column=4
      )
      ...
      c_row += 1
      for _, r in df_current_stock[
          df_current_stock["category"] == cat
      ].iterrows():
        ws_cat.cell(row=c_row, column=1, value=str(r["item_name"]))

        stk_cell = ws_cat.cell(
            row=c_row, column=2, value=float(r["current_stock"])
        )
        stk_cell.number_format = "#,##0.00"

        thresh_cell = ws_cat.cell(
            row=c_row, column=3, value=float(r["min_threshold"])
        )
...
if not st.session_state["logged_in"]:
  ...
  if st.button("Log In"):
    user = cursor.execute(
        ...
        (input_username.strip(), input_password.strip()),
    ).fetchone()
    if user:
...
# --- MAIN APPLICATION (LOGGED IN) ---
  ...
  unread_count = cursor.execute(
      unread_query, (user_name, user_role)
  ).fetchone()[0]
  ...
  if user_role == "Materials Supervisor":
    ...
    pending_requests_count = cursor.execute(
        "SELECT COUNT(*) FROM edit_requests WHERE status = 'PENDING'"
    ).fetchone()[0]
    pending_variances_count = cursor.execute(
        "SELECT COUNT(*) FROM physical_inventory_counts WHERE sync_status ="
        " 'PENDING_ADMIN_REVIEW'"
    ).fetchone()[0]

    edit_option_title = (
        f"✏️ Edit Requests ({pending_requests_count})"
        if pending_requests_count > 0
        else "✏️ Edit Requests"
    )
    audit_option_title = (
        f"📦 Physical Inventory Audit ({pending_variances_count})"
        if pending_variances_count > 0
        else "📦 Physical Inventory Audit"
    )
  ...
  if selected_menu == "📋 Current Inventory":
    ...
    df_items = pd.read_sql_query(
        "SELECT category, item_name, current_stock, min_threshold, unit FROM"
        " master_items ORDER BY category ASC, item_name ASC",
        conn,
    )
    ...
    if not df_items.empty:
      ...
      for cat in categories:
        ...
        df_cat = df_items[df_items["category"] == cat][
            ["item_name", "current_stock", "min_threshold", "unit"]
        ]
  ...
  # --- MENU 2: ANALYTICS DASHBOARD ---
    ...
    df_inv = pd.read_sql_query(
        "SELECT item_name, category, unit, current_stock, min_threshold FROM"
        " master_items",
        conn,
    )
    df_tx_all = pd.read_sql_query(
        "SELECT timestamp, item_name, type, quantity, user_role,"
        " driver_details, project_name FROM transactions",
        conn,
    )
    ...
    if df_inv.empty:
      ...
      if not df_tx_all.empty:
        ...
        df_filtered_tx = df_tx_all[
            df_tx_all["dt_timestamp"] >= start_date
        ].copy()
      else:
        df_filtered_tx = pd.DataFrame()

      total_skus = len(df_inv)
      low_stock_items = df_inv[
          df_inv["current_stock"] <= df_inv["min_threshold"]
      ]
      low_stock_count = len(low_stock_items)

      total_received_qty = (
          df_filtered_tx[df_filtered_tx["type"] == "IN"]["quantity"].sum()
          if not df_filtered_tx.empty
          else 0
      )
      total_issued_qty = (
          df_filtered_tx[df_filtered_tx["type"] == "OUT"]["quantity"].sum()
          if not df_filtered_tx.empty
          else 0
      )
      ...
      m2.metric(
          "Low Stock Alerts",
          low_stock_count,
          delta=f"{low_stock_count} Critical",
          delta_color="inverse" if low_stock_count > 0 else "off",
      )
      m3.metric(
          f"Received (IN) [{time_view.split()[0]}]",
          f"{total_received_qty:,.1f}",
      )
      m4.metric(
          f"Issued (OUT) [{time_view.split()[0]}]", f"{total_issued_qty:,.1f}"
      )
      ...
      if low_stock_count > 0:
        st.error(
            f"⚠️ **Attention Required:** {low_stock_count} item(s) are at or"
            " below safety threshold!"
        )
        st.dataframe(
            low_stock_items[
                ["item_name", "category", "current_stock", "min_threshold", "unit"]
            ],
            use_container_width=True,
        )
      else:
      ...
      if not df_filtered_tx.empty:
        ...
        trend_in = df_filtered_tx_grouped[
            df_filtered_tx_grouped["type"] == "IN"
        ].resample(freq_rule)["quantity"].sum()
        trend_out = df_filtered_tx_grouped[
            df_filtered_tx_grouped["type"] == "OUT"
        ].resample(freq_rule)["quantity"].sum()

        df_trend = pd.DataFrame(
            {"Stock IN": trend_in, "Stock OUT": trend_out}
        ).fillna(0)
      ...
      with c1:
        ...
        df_chart_inv = df_inv.sort_values(
            by="current_stock", ascending=False
        ).head(15)
        st.bar_chart(
            df_chart_inv,
            x="item_name",
      ...
      with c2:
        ...
        cat_summary = (
            df_inv.groupby("category")["item_name"].count().reset_index()
        )
    ...
    if items:
      ...
      with col_a:
        item_selected = st.selectbox(
            "Select Item to Receive", items, key="in_item"
        )
        qty_in = st.number_input(
            "Quantity Received", min_value=0.1, step=1.0, key="in_qty"
        )
      with col_b:
        driver_info = st.text_input(
            "Driver / Delivery Details / DR #",
            placeholder="e.g. John Doe (Plate: ABC-123, DR #9876)",
            key="in_driver",
        )
        remarks_in = st.text_input(
            "General Remarks",
            placeholder="e.g. Verified good condition, unloaded at Bay 2",
            key="in_rem",
        )
      ...
      if photo_mode == "Camera Capture":
        camera_photo = st.camera_input(
            "Take a picture of the delivery / receipt", key="in_camera"
        )
        if camera_photo:
      ...
      if st.button("Submit Stock In", use_container_width=True):
        ...
        cursor.execute(
            "UPDATE master_items SET current_stock = current_stock + ? WHERE"
            " item_name = ?",
            (qty_in, item_selected),
        )
        cursor.execute(
            """
        ...
        conn.commit()
        st.success(
            f"Successfully recorded receiving {qty_in} of {item_selected}!"
        )
        st.rerun()
    else:
  ...
  # --- MENU 4: STOCK OUT ---
    ...
    if items:
      ...
      with col_x:
        item_selected = st.selectbox(
            "Select Item to Issue", items, key="out_item"
        )
        qty_out = st.number_input(
            "Quantity Issued", min_value=0.1, step=1.0, key="out_qty"
        )
        issued_to = st.text_input(
            "Issued To (Person / Subcontractor)",
            placeholder="e.g. Foreman Mike / Subcon ABC",
            key="out_issued_to",
        )

      with col_y:
        driver_out = st.text_input(
            "Driver / Transport Vehicle",
            placeholder="e.g. Driver Bob (Dump Truck #2)",
            key="out_driver",
        )
      ...
      current_stk, min_thresh = cursor.execute(
          "SELECT current_stock, min_threshold FROM master_items WHERE"
          " item_name = ?",
          (item_selected,),
      ).fetchone()
      st.caption(
          f"Current Stock Available: **{current_stk}** | Min Alert Threshold:"
          f" **{min_thresh}**"
      )
      ...
      if st.button("Submit Stock Out", use_container_width=True):
        if qty_out > current_stk:
          st.error(
              f"Cannot issue {qty_out}. Stock available is only {current_stk}."
          )
        else:
          cursor.execute(
              "UPDATE master_items SET current_stock = current_stock - ?"
              " WHERE item_name = ?",
              (qty_out, item_selected),
          )
          cursor.execute(
              """
          ...
          conn.commit()
          st.success(
              f"Successfully issued {qty_out} of {item_selected} to"
              f" {issued_to}!"
          )
          st.rerun()
    else:
  ...
  # --- MENU 5: SYSTEM RESET & TESTING ZONE (ADMIN ONLY) ---
  elif selected_menu == "⚙️ System Reset & Testing Zone":
    st.subheader("⚙️ Developer & System Testing Tools")
    st.warning(
        "⚠️ **Danger Zone:** Wiping software records or resetting databases is"
        " irreversible. Use this before running fresh site tests."
    )

    c_reset1, c_reset2 = st.columns(2)

    with c_reset1:
      st.markdown("### 🧹 1. Soft Reset (Clear Logs Only)")
      st.write(
          "Clears all operational data (**Transactions, Edit Requests,"
          " Notifications, Audit Counts**) while keeping the **Master Inventory"
          " list and current stock counts intact**."
      )
      if st.button("Clear Operational Logs", use_container_width=True):
        cursor.execute("DELETE FROM transactions")
        cursor.execute("DELETE FROM edit_requests")
        cursor.execute("DELETE FROM notifications")
        cursor.execute("DELETE FROM physical_inventory_counts")
        cursor.execute("DELETE FROM reminders")
        cursor.execute("DELETE FROM schedules")
        conn.commit()

        st.success(
            "Operational records cleared! Master inventory counts kept."
        )
        st.rerun()

    with c_reset2:
      st.markdown("### 💣 2. Hard Reset (Full Factory Wipe)")
      st.write(
          "Wipes **ALL database tables**, deletes uploaded proof images,"
          " resets ID sequences back to `1`, and re-seeds default accounts"
          " (`admin`, `supervisor`) & default items."
      )

      confirm_wipe = st.checkbox("I confirm I want to wipe ALL test records.")

      if st.button(
          "🔴 RUN FACTORY HARD RESET",
          type="primary",
        ...
        # 3. Re-seed fresh base configuration
        seed_default_users()
        seed_default_inventory()
        conn.commit()

        st.success(
            "System wiped and restored to default factory test configuration!"
        )
        st.rerun()
